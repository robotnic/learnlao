...
async function main() {
  ...
  console.log(`🎯 Generating TTS for all vocabulary items (male & female voices)\n`);
  ...
  for (let i = 0; i < vocabulary.length; i++) {
    ...
    for (const gender of ['male', 'female']) {
      ...
      console.log(`🎤 [${i + 1}/${vocabulary.length}] ${gender.toUpperCase()}: ${item.id} - "${item.lao}"`);
  ...
  console.log(`   📊 Total processed: ${itemsProcessed}`);
